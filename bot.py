...
async def main():
    ...
    if IS_PRODUCTION_DEPLOYMENT:
        ...
        # The SSH service may stop producing output while connecting.  Do not
        # let an unavailable development tunnel block Telegram polling: its
        # URL-change callback will update the menu button once it is ready.
        try:
            tunnel_url = await asyncio.wait_for(
                asyncio.to_thread(start_tunnel, port=WEBAPP_PORT),
                timeout=5,
            )
        except asyncio.TimeoutError:
            tunnel_url = ""
            logger.warning("SSH-туннель ещё подключается; Telegram polling запускается без ожидания")
        if tunnel_url:
